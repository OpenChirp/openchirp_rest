...
describe('Transducers', function() {
     ...
     */
    describe('/GET/:id transducer', function() {
        it('it should GET a transducer by id', (done) => {
            chai.request(server)
                .get('/api/device/' + theDevice.id + '/transducer/' + theDevice.transducers[0].id)
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.should.be.a('object');
                    res.body.should.have.property('results');
                    // TODO: insert transducer values and test if they are being returned correctly
                    done();
                });
        });
    });
     ...
     */
    describe('/DELETE/:id transducer', function() {
        it('it should DELETE a transducer by id', (done) => {
            chai.request(server)
                .delete('/api/device/' + theDevice.id + '/transducer/' + theDevice.transducers[0].id)
                .end((err, res) => {
                    res.should.have.status(200);
                    done();
                });
